...
public class Q032_BalancedParanthesis {
	...
	private static boolean BalancedParanthesis(String str, String ans) {
		...
		if (ch == '(' || ch == '{' || ch == '[' || ch == '<') {
			return BalancedParanthesis(str.substring(1), ans + ch);
		} else if (ch == ')' || ch == '}' || ch == ']' || ch == '>') {
			if (ans.length() == 0) {
				return false;
			}
			...
			return BalancedParanthesis(str.substring(1), ans);
		} else {
			return BalancedParanthesis(str.substring(1), ans);
		}
