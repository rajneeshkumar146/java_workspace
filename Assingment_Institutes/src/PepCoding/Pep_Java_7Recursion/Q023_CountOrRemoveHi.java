...
public class Q023_CountOrRemoveHi {
	...
	public static void solve() throws Exception {
		...
		String suspect = scn.scanString();
		int ans = RemoveHi(str, new StringBuilder(), suspect);
		pnter.print(ans + "");
	}

	private static int RemoveHi(String str, StringBuilder ans, String suspect) throws IOException {
		if (str.length() == 0) {
			pnter.printLine(ans + "");
			return 0;
		}

		if (str.length() >= suspect.length() && str.substring(0, suspect.length()).equals(suspect)) {
			return 1 + RemoveHi(str.substring(suspect.length()), ans, suspect);
		} else {
			char ch = str.charAt(0);
			return RemoveHi(str.substring(1), ans.append(ch), suspect);
		}
