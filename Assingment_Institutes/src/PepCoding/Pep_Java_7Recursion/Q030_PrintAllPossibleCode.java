...
public class Q030_PrintAllPossibleCode {
	...
	public static void solve() throws Exception {
		...
		int ans = PossibelCodes(str, new StringBuilder(""));
		pnter.print(ans + "");
	}

	private static int PossibelCodes(String str, StringBuilder ans) throws IOException {
		if (str.length() == 0) {
			pnter.printLine(ans + "");
			return 1;
		}
		...
		int oneLengthNum = ch - '1';

		if (oneLengthNum >= 0) { // Inme_mai_se_ek_call_lagni_hi_hai.
			ans.append((char) (oneLengthNum + 'a'));
			count += PossibelCodes(str.substring(1), ans);
		...
		if (str.length() >= 2) {
			int twoLengthNum = Integer.parseInt(str.substring(0, 2));

			if (twoLengthNum > 9 && twoLengthNum < 27) {

				ans.append((char) (twoLengthNum - 1 + 'a'));
				count += PossibelCodes(str.substring(2), ans);
				ans.deleteCharAt(ans.length() - 1);
			}
		}
