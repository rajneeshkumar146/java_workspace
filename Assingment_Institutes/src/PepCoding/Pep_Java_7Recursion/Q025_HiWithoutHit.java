package PepCoding.Pep_Java_7Recursion;
...
public class Q025_HiWithoutHit {
	...
	public static void solve() throws Exception {
		...
		int ans = HiWithoutHit(str, "");
		pnter.printLine(ans + "");
		String s = HiWithoutHitRetuen(str, "");
		pnter.print(s);
	}

	private static int HiWithoutHit(String str, String ans) throws IOException {
		if (str.length() == 0) {
			pnter.printLine(ans + "");
			return 0;
		}
		...
		if (str.length() >= 2 && ch == 'h' && str.charAt(1) == 'i') {
			if (str.length() >= 3 && str.charAt(2) == 't') {
				return HiWithoutHit(str.substring(3), ans + "hit");
			} else {
				return 1 + HiWithoutHit(str.substring(2), ans);
			}
		} else {
			return HiWithoutHit(str.substring(1), ans + ch);
		}

	}

	private static String HiWithoutHitRetuen(String str, String ans) throws IOException {
		if (str.length() == 0) {
			pnter.printLine(ans + "");
			return "";
		}
		...
		if (str.length() >= 2 && ch == 'h' && str.charAt(1) == 'i') {
			if (str.length() >= 3 && str.charAt(2) == 't') {
				s += "hit" + HiWithoutHitRetuen(str.substring(3), ans + "hit");
			} else {
				s += HiWithoutHitRetuen(str.substring(2), ans);
			}
		} else {
			s += ch + HiWithoutHitRetuen(str.substring(1), ans + ch);
		}
