...
public class Q410_ItineraryFromGivenListOfTickets {
	...
	public static void solve() throws Exception {
		...
		HashMap<String, String> DestToSrc = new HashMap<>();
		int n = scn.scanInt();
		for (int i = 0; i < n; i++) {
			String str1 = scn.scanString();
			String str2 = scn.scanString();
			SrcToDest.put(str1, str2);
			DestToSrc.put(str2, str1);
		}

		String Starting = "";

		ArrayList<String> keys = new ArrayList<>(SrcToDest.keySet());
		for (String key : keys) {
			if (!DestToSrc.containsKey(key)) {
				Starting = key;
			}
		}

		while (SrcToDest.containsKey(Starting)) {
			String desti = SrcToDest.get(Starting);
			pnter.printLine(Starting + " ->  " + desti);
			Starting = desti;
		}
	}

	private static void Itinerary(HashMap<Character, Character> map) {
