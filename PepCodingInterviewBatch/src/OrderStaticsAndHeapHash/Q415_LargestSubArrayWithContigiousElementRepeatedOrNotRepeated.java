package OrderStaticsAndHeapHash;
...
public class Q415_LargestSubArrayWithContigiousElementRepeatedOrNotRepeated {
	...
	public static void solve() throws Exception {
		...
		//
		// for (int i = 0; i < num; i++) {
		// map.put(scn.scanInt(), 0);
		// }
		// Set1WithoutDuplicatesMethod1(map); //NotContigious.

		int n = scn.scanInt();
		int[] arr = takeArrayInput(n);
		int ans = Set1WithoutDuplicatesMethod2(arr); // Contigious.
		pnter.printLine(ans + "");

	}

	private static void Set1WithoutDuplicatesMethod1(HashMap<Integer, Integer> map) throws IOException {

		ArrayList<Integer> keys = new ArrayList<>(map.keySet());

		for (Integer key : keys) { // Elements_which_contain_0_Are_Starting.
		...
		for (Integer key : keys) {
			if (map.get(key) == -1) { // Jo_StartingNahiHaiUsseKuchNahiMilegaSoIgnoreIt.
				continue;
			}

			int size = 0;
			int suspect = key;
			while (map.containsKey(suspect)) {
				size++;
				suspect++;
			}
			OverallMax = Math.max(OverallMax, size);
		}

		pnter.print(OverallMax + "");

	}

	private static int Set1WithoutDuplicatesMethod2(int[] arr) {
		int length = 0;
	...
	private static int Set2WithDuplicates(int[] arr) {

		int length = 0;

		for (int i = 0; i < arr.length; i++) { // everyElement_has_chanceToMakeContigiousSubArray.
