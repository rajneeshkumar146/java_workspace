...
public class Q413_countOfAllDistinctElementInEveryWindow {
	...
	public static void solve() throws Exception {
		...
		DistinctWindow(arr, k);

	}

	private static void DistinctWindow(int[] arr, int k) throws IOException {

		HashMap<Integer, Integer> map = new HashMap<>();
		for (int i = 0; i < k; i++) {
			PutInHashMap(map, arr[i]);
		}

		pnter.printLine(map.keySet() + "");

		for (int i = 0, j = k; j < arr.length; i++, j++) {
			int OutGoingEle = arr[i];
			int ComingEle = arr[j];

			RemoveFromMap(map, OutGoingEle);
			PutInHashMap(map, ComingEle);
	...
	private static void PutInHashMap(HashMap<Integer, Integer> map, int i) {
	...
	private static void RemoveFromMap(HashMap<Integer, Integer> map, int val) {
