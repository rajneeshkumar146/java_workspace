...
public class Q409_NumberOfEmployees {
	...
	public static void solve() throws Exception {
		...
		for (int i = 0; i < n; i++) {
			char Employee = scn.scanString().charAt(0);
			char Manager = scn.scanString().charAt(0);
			PutInHashMap(map, Manager, Employee);
		}
		...
		for (Character key : keys) {
			int index = key - 'A';
			freqMap[index] = count(map, freqMap, key);
			pnter.printLine(key + " " + freqMap[index]);
		}

	}

	private static int count(HashMap<Character, HashSet<Character>> map, int[] freqMap, Character key) {
		if (!map.containsKey(key)) {
			return 0;
		}

		if (freqMap[key - 'A'] != 0) {
			return freqMap[key - 'A'];
		}

		int count = 0;
		ArrayList<Character> ele = new ArrayList<>(map.get(key));
		for (Character ch : ele) {
			if (ch == key) {
				continue;
			}

			count += count(map, freqMap, ch) + 1;
		}
		return count;
	}

	private static void PutInHashMap(HashMap<Character, HashSet<Character>> map, char Manager, char Employee) {

		if (!map.containsKey(Manager)) {
			map.put(Manager, new HashSet<>());
