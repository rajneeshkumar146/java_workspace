...
public class Q416_GroupShifted {
	...
	public static void solve() throws Exception {
		...
		GetShiftedStrings(arr);

	}

	private static void GetShiftedStrings(String[] arr) throws IOException {
		HashMap<Integer, HashSet<String>> map = new HashMap<Integer, HashSet<String>>();
		for (int i = 0; i < arr.length; i++) {
			int Shiftcode = getCode(arr[i]);
			PutInHashMap(map, Shiftcode, arr[i]);
		}

		ArrayList<Integer> keys = new ArrayList<>(map.keySet());
		for (Integer key : keys) {
			pnter.printLine(map.get(key) + "");
		}
	...
	private static int getCode(String str) {
		...
		for (int i = 1; i < str.length(); i++) {
			...
			int diff = (ch2 - ch1 + 26) % 26; // NumberOfCharacterBetweenThem.
			ans = ans * pow + diff;
			pow *= 10;
		}
	...
	private static void PutInHashMap(HashMap<Integer, HashSet<String>> map, Integer key, String value) {
