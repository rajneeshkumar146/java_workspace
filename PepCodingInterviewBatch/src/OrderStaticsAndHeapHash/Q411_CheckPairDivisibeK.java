...
public class Q411_CheckPairDivisibeK {
	...
	public static void solve() throws Exception {
		...
		boolean res = CheckPair(arr, k);
		pnter.print(res + "");

	}

	private static boolean CheckPair(int[] arr, int k) {
		if (arr.length % 2 != 0) {
			return false;
		}

		HashMap<Integer, HashSet<Integer>> map = new HashMap<>();
		for (int val : arr) { // remainder_ke_across_values_rakhLi.
			int rem = val % k;
			PutInHashMap(map, rem, val);
		}

		ArrayList<Integer> keys = new ArrayList<>(map.keySet());
		for (Integer key : keys) {
			// AgarDonoKaLengthSameNahiHaiTohReturnFalse.
			// agar(k-key)==key.TohUssSetKaSSizeEvenHonaChahiye.
			// (k-key)MeraMapMapExistKrnaChahiye.

			int len1 = map.get(key).size();
			int otherNumber = k - key;

			if (otherNumber == key && map.get(otherNumber).size() % 2 != 0) {
				return false;
			}
			...
			int len2 = map.get(otherNumber).size();
			if (len1 != len2) {
				return false;
			}
	...
	private static void PutInHashMap(HashMap<Integer, HashSet<Integer>> map, int key, int val) {
