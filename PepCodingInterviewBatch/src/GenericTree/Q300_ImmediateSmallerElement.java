...
public class Q300_ImmediateSmallerElement {
	...
	public static void solve() throws Exception {
		...
		gt.display();
		pnter.printLine(gt.ImmediateSmaller(30) + "");
	...
	private static class GenericTree {
		...
		public int ImmediateSmaller(int x) throws IOException {
			return ImmediateSmaller(this.root, x);
		}

		private int ImmediateSmaller(Node node, int x) throws IOException {

			int realVal = Integer.MIN_VALUE;
			for (Node child : node.children) {
				int val = ImmediateSmaller(child, x);
				realVal = Math.max(val, realVal);
			}

			if (node.data < x) {
				if (node.data > realVal) {
					realVal = node.data;
				}
			}
			return realVal;
