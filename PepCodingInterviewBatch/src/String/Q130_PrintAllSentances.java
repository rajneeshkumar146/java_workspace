...
public class Q130_PrintAllSentances {
	...
	public static void solve() throws Exception {
		int n = scn.scanInt();
		int m = scn.scanInt();
		String[][] word = new String[n][m];

		for (int i = 0; i < n; i++) {
			int k = scn.scanInt();

			for (int j = 0; j < k; j++) {
				word[i][j] = scn.scanString();
			}
		}

		AllSetances(word, n, m, 0, "");

	}

	private static void AllSetances(String[][] arr, int n, int m, int row, String ans) throws IOException {
		if (row >= n) {
			pnter.printLine(ans);
			return;
		}
		for (int j = 0; j < m; j++) {
			if (arr[row][j] != null) {
				AllSetances(arr, n, m, row + 1, ans + arr[row][j] + " ");
			} else {
				AllSetances(arr, n, m, row + 1, ans + " ");
			}
		}
	}
