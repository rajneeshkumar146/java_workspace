...
public class Q127_RepeatedSubsequenceOfLengthOf2 {
	...
	public static void solve() throws Exception {
		...
		boolean ans = RepeatedSubsequence(str);
		pnter.print(ans + "");
	}

	private static boolean RepeatedSubsequence(String str) throws IOException {
		int[] freqmap = FreqMapOfStringof26(str);
		...
		for (int i = 0; i < str.length(); i++) {
			...
			if (freqmap[index] > 2) {
				return true;
			} else if (freqmap[index] != 1) {
				sb.append(ch);
			}
		}

		str = sb.toString();
		if (IsPalindrome(str)) { // IfAllCharacterIsUniqueThenStringBecomeEmpty.EmptyStringIsPalindrome.
			return false;
		}
