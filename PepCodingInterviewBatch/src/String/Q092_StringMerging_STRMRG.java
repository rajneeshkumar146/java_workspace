...
public class Q092_StringMerging_STRMRG {
	...
	public static void solve() throws Exception {
		...
		while (t-- > 1) {
			int n = scn.scanInt();
			int m = scn.scanInt();
			String sn = scn.scanString();
			String sm = scn.scanString();

			int min = StringMerging(sn, n, sm, m);
			pnter.printLine(min + "");
		}
	...
	private static int StringMerging(String sn, int n, String sm, int m) throws IOException {
		sn = compress(sn, n);
		sm = compress(sm, m);
		...
		int LCS = LCSLength(sn, n, sm, m);

		return n + m - LCS;
	}

	private static int LCSLength(String sn, int n, String sm, int m) throws IOException {

		int[][] LCSDP = new int[n + 1][m + 1];

		for (int row = n - 1; row >= 0; row--) {
			for (int col = m - 1; col >= 0; col--) {
				if (sn.charAt(row) == sm.charAt(col)) {
					LCSDP[row][col] += LCSDP[row + 1][col + 1] + 1;
				} else {
					LCSDP[row][col] = Math.max(LCSDP[row + 1][col], LCSDP[row][col + 1]);
				}
			}
		}
