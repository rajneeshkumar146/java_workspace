...
public class Q097_TotalAnagramSubstrings {
	...
	public static void solve() throws Exception {
		...
		int ans = AnagramSubstring(str);
		pnter.print(ans + "");

	}

	private static int AnagramSubstring(String str) {
		HashMap<String, Integer> map = new HashMap<>();
		for (int i = 0; i < str.length(); i++) {

			int[] freq = new int[26];

			for (int j = i; j < str.length(); j++) { // Loop_for_generateSubstrings.
				StringBuilder sb = new StringBuilder();
				freq[str.charAt(j) - 'a']++;

				for (int k = 0; k < 26; k++) { // Loop_for_making_code_handTohand.
					if (freq[k] != 0) {
						sb.append((char) (k + 'a'));
						sb.append(freq[k]);
					}
				...
				if (!map.containsKey(code)) {
					map.put(code, 1); // Map_mai_Code_ko_daldiya.
