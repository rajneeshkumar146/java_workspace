...
public class Q090_LaxicographicalFirstPalindrome {
	...
	public static void solve() throws Exception {
		...
		LaxicographicalPalindrome(str);

	}

	private static void LaxicographicalPalindrome(String str) throws IOException {
		...
		int oddCount = 0; // OddCount_must_be_zero.
		int oddIndex = -1;
		...
		if (oddCount > 1) {
			pnter.print("Not Possile");
			return;
		}

		int leng = str.length();
		char[] strarr = new char[leng];

		if (oddCount == 1) {
			strarr[leng / 2] = (char) (oddIndex + 'a');
			fre[oddIndex]--; // ifIsOddThenReduceIt.if_is_more_then_downCode_handel_it.
		}

		int left = 0; // String_dono_trf_se_bnate_chlenge.
		int right = leng - 1;

		for (int i = 0; i < fre.length; i++) { // ForwardLoop.
			if (fre[i] > 0) {
				int EvenChar = fre[i];
				for (int j = 1; j <= EvenChar; j += 2) {
					strarr[left++] = ((char) (i + 'a'));
					strarr[right--] = ((char) (i + 'a'));
				}
			}
		}

		for (int i = 0; i < leng; i++) {
			pnter.print(strarr[i] + "");
		}
