...
public class Q078_GroupWordOfSameTogether {
	...
	public static void solve() throws Exception {
		HashMap<Integer, HashSet<String>> mapedSet = new HashMap<>();
		// int n = scn.scanInt();

		String[] word = { "may", "student", "students", "dog", "studentssess", "god", "cat", "act", "tab", "bat",
		...
		for (int i = 0; i < word.length; i++) {
			...
			for (int j = 0; j < str.length(); j++) {
				mapedNum |= (1 << (str.charAt(j) - 'a'));
			}

			if (!mapedSet.containsKey(mapedNum)) {
				mapedSet.put(mapedNum, new HashSet<>());
				mapedSet.get(mapedNum).add(str);
			} else {
				mapedSet.get(mapedNum).add(str);
			}
		...
		ArrayList<Integer> list = new ArrayList<>(mapedSet.keySet());

		for (Integer key : list) {
			pnter.printLine(mapedSet.get(key) + "");
		}
