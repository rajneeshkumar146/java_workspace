...
public class Q088_permpel_codechef {
	...
	public static void solve() throws Exception {
		...
		while (t-- > 0) {
			...
			int[] ans = StringToPalindromePermutation(str);
			if (ans[0] == -1) {
				pnter.printLine("-1");
			} else {
				displayArray(ans);
	...
	private static int[] StringToPalindromePermutation(String str) {
		...
		for (int i = 0; i < arr.length; i++) {
			if (arr[i] != null) {
				if (!flag && arr[i].size() % 2 != 0) {
					flag = true;
					oddcount++;
					oddIndex = i;
				} else if (flag && arr[i].size() % 2 != 0) {
					ans[0] = -1;
					return ans;
				}
			}
		...
		if (oddcount > 1) {
			ans[0] = -1;
			return ans;
		} else if (oddcount != 0 && str.length() % 2 == 0) {
			ans[0] = -1;
			return ans;
		}

		if (oddcount == 1) {
			ArrayList<Integer> list = arr[oddIndex];
			int size = list.size();
			ans[str.length() / 2] = list.get(size / 2);

			int left = str.length() / 2 - 1;
			int right = str.length() / 2 + 1;

			int listLeft = size / 2 - 1;
			int listRight = size / 2 + 1;

			int count = 1;

			while (count < size - 1) {

				ans[left] = list.get(listLeft);
				ans[right] = list.get(listRight);
		...
		int ansLeft = 0;
		int ansRight = str.length() - 1;

		for (int i = 0; i < arr.length; i++) {
			if (arr[i] != null) {
				ArrayList<Integer> key = arr[i];
				int left = 0;
				int right = key.size() - 1;

				while (left < right) {
					ans[ansLeft] = key.get(left);
					ans[ansRight] = key.get(right);
