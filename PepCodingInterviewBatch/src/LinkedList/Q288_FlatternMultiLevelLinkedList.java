...
public class Q288_FlatternMultiLevelLinkedList {
	...
	private static class Linkedlist {
		...
		public void FlatternDFSRecur() throws IOException {
			FlatternDFSRecur(this.head);
		}

		private void FlatternDFSRecur(Node node) throws IOException {
			if (node == null) {
				return;
			}

			pnter.print(node.data + " ");
			FlatternDFSRecur(node.Verti);
			FlatternDFSRecur(node.next);

		}

		public void FlatternDFSIrr() throws IOException {
			LinkedList<Node> stack = new LinkedList<>(); // AddLast_and_RemoveFirst.

			stack.addLast(this.head);
