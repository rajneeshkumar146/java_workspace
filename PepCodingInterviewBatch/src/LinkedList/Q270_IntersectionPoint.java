...
public class Q270_IntersectionPoint {
...
//		System.out.println("First LL Size");
		int n = scn.scanInt();
		while (n-- > 0) {
			ll.add(scn.scanInt());
		}

//		System.out.println("Second LL Size");
		int m = scn.scanInt();
		while (m-- > 0) {
			ll.add2(scn.scanInt());
		}
		...
		int range = ((n - 0) + 1) + 0;
		 ll.InsertIntersectionLiner((int) (Math.random() * range));

		// int ans = ll.DetectIntersectionMethod1();
		// pnter.print(ans + "");

		ll.InsertIntersectionCycle();
		int ans = ll.IntersectionNodeMethod2();
		pnter.print(ans + "");
	...
	private static class LinkedList {
		...
		public void InsertIntersectionLiner(int n) {
			Node node = getNode(n);
			this.tail2.next = node;
		}

		public int DetectIntersectionMethod1() {
		...
		private Node IsCyclePresent() {
			...
			return isLoop ? fast : null;

		}

		public void InsertIntersectionCycle() {
			this.tail.next = this.head2;
		}

		private Node DetectIntersectionMethod2() {

			Node CycleNode = IsCyclePresent();
			if (CycleNode == null) {
				return null;
			}
		...
		public int IntersectionNodeMethod2() {
			Node node = DetectIntersectionMethod2();
			return node == null ? -1 : node.data;
		}
