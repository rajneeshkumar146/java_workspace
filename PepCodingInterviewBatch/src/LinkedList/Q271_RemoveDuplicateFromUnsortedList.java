...
public class Q271_RemoveDuplicateFromUnsortedList {
...
//		pnter.print(RemoveDupliByHashMap(ll) + "");
		
		pnter.print(RemoveDupliBySorting(ll)+"");
	}

	private static LinkedList<Integer> RemoveDupliBySorting(LinkedList<Integer> ll) {
		...
		int size = ll.size();
		nl.addLast(ll.removeFirst());
		for (int i = 1; i < size; i++) {
			if (nl.getLast() != ll.getFirst()) {
				nl.addLast(ll.removeFirst());
			}else{
				ll.removeFirst();
			}
	...
	private static LinkedList<Integer> RemoveDupliByHashMap(LinkedList<Integer> ll) {
		LinkedList<Integer> nl = new LinkedList<>();
		LinkedHashMap<Integer, Integer> map = new LinkedHashMap<>();

		int size = ll.size();
		for (int i = 0; i < size; i++) {
			PutInHashMap(map, ll.removeFirst());
		}
	...
	private static void PutInHashMap(LinkedHashMap<Integer, Integer> map, int key) {
