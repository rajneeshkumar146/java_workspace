...
public class Q076_GreatestProductInarrayWhoseFactorExistinArray {
	...
	public static void solve() throws Exception {
		...
		int res = GreatestProduct(arr, n);
		pnter.print(res + "");

	}

	private static int GreatestProduct(int[] arr, int n) {
		Arrays.sort(arr);
		...
		for (int i = n - 1; i > 0; i--) {
			for (int j = 0; j < i && arr[j] * arr[j] <= arr[i]; j++) {
				...
				if (arr[i] % arr[j] == 0) { // Valid_first_factor.
					int ESF = arr[i] / arr[j]; // ExpectedSecondFactor.

					if (set.containsKey(ESF) && ESF != arr[j]) { // Unequal_factors.
						if (ESF == arr[i] && set.get(ESF) == 1) {
							return -1;
						}

						return arr[i];

					} else if (set.containsKey(ESF) && ESF == arr[j]) { // Equal_Factors.Like_2*2=4.
