...
public class Q064_MinCostToConvergeArrayElement {
	...
	public static void solve() throws Exception {
		...
		// median. This is because the univariate median also minimizes the sum
		// of distances from the points.
		
		int median = arr[arr.length >> 1];
		if (arr.length % 2 != 0) {
			median = (arr[arr.length >> 1] + arr[(arr.length - 1) >> 1]) >> 1;
		}

		int sum = 0;

		for (int i = 0; i < n; i++) {
			sum += Math.abs(median - arr[i]);
		}
