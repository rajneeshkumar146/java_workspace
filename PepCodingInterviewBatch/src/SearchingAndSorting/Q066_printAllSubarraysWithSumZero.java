...
public class Q066_printAllSubarraysWithSumZero {
	...
	public static void solve() throws Exception {
		...
		AllSubarrayByUsingHasMap(arr, n);

	}

	private static void AllSubarrayByUsingHasMap(int[] arr, int n) throws IOException {

		int[] preffixArray = PrifixSumArray(arr);
		...
		for (int i = 0; i < n; i++) {
			if (!map.containsKey(preffixArray[i])) {
				map.put(preffixArray[i], new HashSet<>());
			}

			map.get(preffixArray[i]).add(i);

		}

		pnter.print(map.get(9) + "");
