...
public class Q068_ProductArrayPuzzle {
	...
	public static void solve() throws Exception {
		...
		int[] suffixProductArray = ProductArrays(arr, true);
		int[] preffixProductArray = ProductArrays(arr, false);

		for (int i = 0; i < n; i++) {
			arr[i] = suffixProductArray[i] * preffixProductArray[i];
		}

		// displayArray(0, arr.length-1, suffixProductArray);
		// displayArray(0, arr.length-1, preffixProductArray);

		displayArray(0, arr.length - 1, arr);
	}

	private static int[] ProductArrays(int[] arr, boolean suffix) throws IOException {

		int[] narr = new int[arr.length];
		if (suffix) {
			narr[0] = 1;
			for (int i = 1; i < arr.length; i++) {
				narr[i] = arr[i - 1] * narr[i - 1];
			}
		} else {
			narr[arr.length - 1] = 1;
