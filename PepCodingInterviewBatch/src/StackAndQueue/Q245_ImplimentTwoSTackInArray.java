...
public class Q245_ImplimentTwoSTackInArray {
	// public static Scanner scn = new Scanner(System.in);
	private static Scan scn = new Scan(System.in);
	private static Print pnter = new Print();

	private static int[] s;
	private static int tos1 = -1;
	private static int tos2 = s.length;

	Q245_ImplimentTwoSTackInArray(int n) {
		s = new int[n];
	}

	private static void push1(int item) throws Exception {
		if (tos1 + 1 == tos2) {
	...
	private static void push2(int item) throws Exception {
		if (tos2 - 1 == tos1) {
			throw new Exception("StackOverFlow");
		}
	...
	private static int pop1() throws Exception {
		if (tos1 == -1) {
	...
	private static int pop2() throws Exception {
		if (tos2 == s.length) {
			throw new Exception("StackIsEmpty");
		}
		int rv = s[tos2];
	...
	private static int size2() {
		return s.length - tos2;

	}

	private static void display1() throws IOException {
	...
	private static void display2() throws IOException {
		for (int i = tos2; i > tos1; i--) {
			pnter.print(s[i] + " ");
		}
