...
public class Q259_RottenAllOranges {
	...
	public static void solve() throws Exception {
		...
		// 3 5 2 1 0 2 1 0 0 1 2 1 1 0 0 2 1
		// 3 5 2 1 0 2 1 1 0 1 2 1 1 0 0 2 1

		int[][] arr = takeMatrixInput(scn.scanInt(), scn.scanInt());

		int time = RottenOranges(arr);
		pnter.print(time == 0 ? "Not Possible" : time + "");

	}

	private static int RottenOranges(int[][] arr) {

		LinkedList<Pair> Queue = new LinkedList<>(); // addLast,RemoveFirst.

		for (int i = 0; i < arr.length; i++) { // PreProcessStack.
		...
		while (!Queue.isEmpty()) {
			...
			NewRottenPositions(Queue, arr, x, y);

		}

		for (int i = 0; i < arr.length; i++) { // CheckAllArangesAreRottenOrNot.
			for (int j = 0; j < arr[0].length; j++) {
				if (arr[i][j] == 1) {
					return 0;
				}
	...
	private static void NewRottenPositions(LinkedList<Pair> Queue, int[][] arr, int x, int y) {
		int[][] dir = Eightdirction();

		for (int i = 0; i < dir.length; i++) {
			int x1 = dir[i][0];
			int y1 = dir[i][1];

			if (x + x1 < 0 || x + x1 >= arr.length || y + y1 < 0 || y + y1 >= arr[0].length) {
				continue;
			}

			if (arr[x + x1][y + y1] == 1) {
				arr[x + x1][y + y1] = 2;
				Queue.addLast(new Pair(x + x1, y + y1));
			}
		}
