...
public class Q478_CountAllSetBitFrom1ToN {
	...
	public static void solve() throws Exception {
		...
		for (int i = 1; i <= count; i++) {
			...
			int cluster = ((n + 1) >> i);
			int setBitsInCluster = cluster * (1 << (i - 1));
			ans += setBitsInCluster;
			int rem = (n + 1) % (1 << i);

			int totalBitsInCluster = 1 << i;

			if (rem > (totalBitsInCluster >> 1)) {
				ans += rem - (totalBitsInCluster >> 1);
			}
		}
