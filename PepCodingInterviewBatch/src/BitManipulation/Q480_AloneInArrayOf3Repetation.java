...
public class Q480_AloneInArrayOf3Repetation {
	...
	private static int AloneArray(int[] arr, int n) {
		HashSet<Integer> set = new HashSet<Integer>();

		int sum = 0;
		for (int i = 0; i < n; i++) {
			sum += arr[i];
			set.add(i); // Now_all_elements_present_but_count_is_only_1.
		}
		...
		return (sum - 3 * Unique_sum) / 2;
	}

	private static int AloneArraySetBit(int[] arr, int n) {
