...
public class Q469_CountOfSetBit {
	...
	private static int SetBit(int n) {
		...
		while (n > 0) { // when_all_become_one_it_become_negtive_or_either_all_become_zero.
			count += n & 1;
			n = n >> 1;
		}
	...
	private static int SetBit_Brian_Kernighann_Algorithm(int n) {
		int count = 0;
		while (n > 0) {
			n = n & n - 1;
			count++;
