...
public class Q143_FindAllCommomElements {
	...
	public static void solve() throws Exception {
		...
		SerachAllCommomElements(arr, n, m);
	}

	// -------------------------------------------------------------------------------------------------------------

	private static void SerachAllCommomElements(int[][] arr, int n, int m) throws IOException {

		HashMap<Integer, HashMap<Integer, Integer>> map = new HashMap<Integer, HashMap<Integer, Integer>>();

		for (int col = 0; col < m; col++) {
			PutInHashMap(map, arr[0][col], 0);
		}
		...
		for (int row = 1; row < n; row++) {
			for (int col = 0; col < m; col++) {
				...
				if (map.containsKey(arr[row][col])) {
					if (!map.get(arr[row][col]).containsKey(row)) {
						map.get(arr[row][col]).put(row, 1);
					} else {
						int fre = map.get(arr[row][col]).get(row);
						map.get(arr[row][col]).put(row, fre + 1);
	...
	private static void PutInHashMap(HashMap<Integer, HashMap<Integer, Integer>> map, int key, int index) {

		if (!map.containsKey(key)) {
			map.put(key, new HashMap<>());
			map.get(key).put(index, 1);
		} else {
			int fre = map.get(key).get(index);
			map.get(key).put(index, fre + 1);
		}
