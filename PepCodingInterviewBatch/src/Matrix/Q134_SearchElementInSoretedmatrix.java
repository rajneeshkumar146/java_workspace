...
public class Q134_SearchElementInSoretedmatrix {
	...
	public static void solve() throws Exception {
		...
		int res = SerachElement(arr, n, m);
		pnter.print(res + "");
	}

	private static int SerachElement(int[][] arr, int n, int m) {
		...
		for (int col = m - 1; col >= 0; col--) {
			...
			for (int row = 0; row < n; row++) {
				...
				boolean searchRes = BinarySearch(arr[row], m, arr[r][col]);
				if (!flag && searchRes) {
					res = arr[r][col];
					flag = true;
				}
	...
	private static boolean BinarySearch(int[] ar, int m, int ele) {
