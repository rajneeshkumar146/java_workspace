...
public class Q138_TicTacToe {
	...
	public static void solve() throws Exception {
		...
		for (int i = 0; i < board.length; i++)
			board[i] = scn.scanString().charAt(0);

		boolean res=TicTacToe(board, n);
		
		pnter.print(res+"");

	}

	private static boolean TicTacToe(char[] board, int n) {

		// Wining-Situation_indexes.
		...
		if (countx == count0 || countx == (count0 + 1)) {
			...
			// If_0inWinningSituation
			if (IsCharWin(board, win, ch2)) {

				// If_XinalsoWinningSituation.
				if (IsCharWin(board, win, ch1)) {
					return false; // Notpossible_bothAreWinnerAtSameTime.
				} else {
					return countx == count0; // agar_XneFirstTimeAtaHaiToTabhiJeetegaJabDonokaCountWqualHoga.
				}

			} else {

				if(IsCharWin(board, win, ch1) && countx!= count0){ //IfCh1IswinnerthenthereCountCan'tBeEquual.
					return false;
				}else{
					return true;
				}
	...
	private static boolean IsCharWin(char[] board, int[][] WS, char ch) {

		for (int row = 0; row < WS.length; row++) {
			if (board[WS[row][0]] == ch && board[WS[row][1]] == ch && board[WS[row][2]] == ch) {
				return true;
			}
		}
		return false;
