...
public class Q05_Circle_City {
	...
	public static void main(String[] args) throws Exception {
		int T = scn.scanInt();
		while (T-- > 1) {
			pnter.printLine(solve() ? "possible" : "impossible");
		}
		pnter.print(solve() ? "possible" : "impossible");
		pnter.close();
	}

	public static boolean solve() throws Exception {
//special case: 18 0 : (3,3)
		long D = scn.scanLong();
		long k = scn.scanLong();
		long count = 0;

		for (int i = 0; i * i < D; i++) {
			int x = (int) Math.sqrt(D - i * i);
			if (x * x == D - i * i) {
				count++;
			}
